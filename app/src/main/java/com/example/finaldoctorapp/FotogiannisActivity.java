...
/**
 * Copyright (C) 2023 Evangelos Vogiatzidakis
 * All rights reserved.
 */

public class FotogiannisActivity extends AppCompatActivity {
    //This Class is About doctor Fotogiannis
    ...
    private final String classID="ΦΩΤΟΓΙΑΝΝΗΣ ΧΑΡΑΛΑΜΠΟΣ";

    private final String doctorCategory="ΔΕΡΜΑΤΟΛΟΓΟΣ";
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        setContentView(R.layout.activity_fotogiannis);

        likebutton = findViewById(R.id.likeButtonFotog);
                ...
                if(!likedDoctorBool) {
                    usersRef.child(userKey).child("likedDoctors").addListenerForSingleValueEvent(new ValueEventListener() {
                                        ...
                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                            ...
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()) {
                                                    likebutton.setImageResource(R.drawable.images_red);
                                                    likedDoctorBool=true;
                                                    Toast.makeText(FotogiannisActivity.this, "Successful", Toast.LENGTH_SHORT).show();
                                                }
                                            }
                                        });
                            }
                        ...
                        public void onCancelled(@NonNull DatabaseError databaseError) {
                            Toast.makeText(FotogiannisActivity.this, "Error: " + databaseError.getMessage(), Toast.LENGTH_SHORT).show();
                        }
                    });
                }else{
                    usersRef.child(userKey).child("likedDoctors").addListenerForSingleValueEvent(new ValueEventListener() {
                            ...
                            if (dataSnapshot.exists() && dataSnapshot.hasChildren()) {
                                ...
                                usersRef.child(userKey).child("likedDoctors").setValue(likedDoctor)
                                        .addOnCompleteListener(new OnCompleteListener<Void>() {
                                            ...
                                            public void onComplete(@NonNull Task<Void> task) {
                                                if (task.isSuccessful()) {
                                                    Toast.makeText(FotogiannisActivity.this, "Succesfull", Toast.LENGTH_SHORT).show();
                                                    likebutton.setImageResource(R.drawable.images);
                                                    likedDoctorBool=false;
                                                }else {
                                                    Toast.makeText(FotogiannisActivity.this, "Failed to remove: " + task.getException().getMessage(), Toast.LENGTH_SHORT).show();
                                                }
                                            }
                                        });
                            }
                        ...
                        if (dataSnapshot.exists() && dataSnapshot.hasChildren()) {
                            ...
                            usersRef.child(userKey).child("appointmentHistory").setValue(appointmentHistory)
                                    .addOnCompleteListener(new OnCompleteListener<Void>() {
                                        ...
                                        public void onComplete(@NonNull Task<Void> task) {
                                            if (task.isSuccessful()) {
                                                //Toast.makeText(FotogiannisActivity.this, "Success!\nValue1: " + value1 + "\nValue2: " + value2, Toast.LENGTH_LONG).show();
    ...
    private void updateCloseDate(String newClosedDate) {
        ...
        doctorRef.push().setValue(newClosedDate)
                .addOnSuccessListener(aVoid -> {
                    Toast.makeText(FotogiannisActivity.this, "Successfully add the new appointment of user.", Toast.LENGTH_SHORT).show();
                })
    ...
    public void openMyProfileActivity(View view) {
        ...
        intent.putExtra("useID",emailChildString);
